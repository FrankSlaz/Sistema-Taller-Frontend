...
interface ClienteSelectProps {
  value: number | null;
  onChange: (clienteId: number, clienteNombre: string) => void;
  initialLabel?: string | null;
  error?: string;
}

/**
 * Combobox de Cliente: busca en /clientes mientras el usuario escribe.
 * Se usa en el formulario de Equipos y, más adelante, en Reparaciones.
 */
export default function ClienteSelect({ value, onChange, initialLabel, error }: ClienteSelectProps) {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['clientes-select', term],
    queryFn: () => clientesApi.findAll({ search: term, limit: 8 }),
    enabled: open,
  });
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        <span className={value ? 'text-graphite-900' : 'text-graphite-400'}>
          {value ? selectedLabel || `Cliente #${value}` : 'Seleccionar cliente…'}
        </span>
                ...
                <li key={cliente.id}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectedLabel(cliente.nombre);
                      setOpen(false);
                      setTerm('');
                    }}
                  ...
                  >
                    <span>
                      ...
                      {cliente.numeroDocumento && (
                        <span className="ml-1 text-xs text-graphite-400">({cliente.numeroDocumento})</span>
                      )}
                    </span>
                    {cliente.id === value && <Check size={14} className="text-graphite-900" />}
                  </button>
                </li>
              ))}
          </ul>
